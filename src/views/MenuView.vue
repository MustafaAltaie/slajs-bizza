...
<template>
  <div>
    ...
    <div v-else>
      <ol>
        <li v-for="type in typeList" :key="type" @click="scrollTo(`${type}Menu`)">{{ type.toUpperCase() }}</li>
      </ol>
      <div v-for="(items, type) in groupedMenu" :key="type" class="menu-section">
        ...
        <ul :id="`${type}Menu`">
          <li v-for="item in items" :key="item.id">
            ...
            <small><span>Ingredients:</span> {{ item.toppings && Array.isArray(item.toppings) ? item.toppings.join(', ') : 'No toppings' }}</small>
